#!/bin/bash
# Builds TooMany.app and packs it into a drag-to-install DMG.
set -euo pipefail
cd "$(dirname "$0")"
...
DMG="dist/TooMany-$VERSION.dmg"
STAGING=$(mktemp -d)
RW="$STAGING/rw.dmg"
trap 'rm -rf "$STAGING"' EXIT

mkdir "$STAGING/root"
cp -R dist/TooMany.app "$STAGING/root/"
ln -s /Applications "$STAGING/root/Applications"
cp packaging/AppIcon.icns "$STAGING/root/.VolumeIcon.icns"

hdiutil create -volname "TooMany" -srcfolder "$STAGING/root" -ov -format UDRW "$RW" >/dev/null
...
echo "first time (or: xattr -d com.apple.quarantine /Applications/TooMany.app)."
